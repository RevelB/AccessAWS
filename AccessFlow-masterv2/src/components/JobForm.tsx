...
import { getFirestore, collection, addDoc, doc, updateDoc, deleteDoc, getDoc, setDoc } from 'firebase/firestore';
...
export function JobForm({ mode, initialData, viewOnly = false }: JobFormProps) {
    ...
    if (user) {
      const fetchUserPrefs = async () => {
        ...
        if (docSnap.exists()) {
          ...
          if (mode === 'create' && data.initials && !form.getValues('creator')) {
            form.setValue('creator', data.initials);
          }
          if (data.jobFormServiceHeight) {
            setScrollAreaHeight(data.jobFormServiceHeight);
            previousHeightRef.current = data.jobFormServiceHeight;
          }
        }
      };
      fetchUserPrefs();
  ...
  const handleResizeEnd = useCallback(async () => {
    ...
    if (newHeight !== previousHeightRef.current) {
        ...
        try {
            const userPrefsRef = doc(db, 'userprefs', user.uid);
            await setDoc(userPrefsRef, { jobFormServiceHeight: newHeight }, { merge: true });
        } catch (error) {
            console.error("Failed to save scroll area height:", error);
            // Optionally show a toast error, but might be too noisy for this feature.
        }
    }
  }, [user, db]);
  ...
  const handleDeleteJob = async () => {
    ...
    try {
      ...
      const deletedJobData = {
        ...initialData,
        deletedAt: new Date().toISOString(),
        userWhoDeleted: user.email || 'unknown',
      };
      const deletedJobsCollectionRef = collection(db, 'deletedjobs');
      // Use the same ID for the deleted job doc for easy reference if needed
      await setDoc(doc(deletedJobsCollectionRef, initialData.id), deletedJobData);
